...
from libs.BleMeasDataCollector import *
from libs.util import *
from libs.connected_mode import ConnectedStateMachine
import logging
import datetime
import traceback
import time
...
class DataLogger():
    ...
    def GetDate(self):
        ...
        return today.strftime(r'%Y-%m-%d_%H-%M-%S')
    ...
    def CreateDataLogger(self,DataName = None):
        ...
        if self.logformat == 'csv':
            with open(self.datalogfilen, 'w') as dfileh:
                dfileh.write('date_time,address,RSSI,pressure,temperature,z_acc_low,z_acc_high,x_acc_low,x_acc_high,batt_voltage,raw_data,error_code,counter,appearance\n')
    ...
    def ContinuousLogging(self):
        ...
        while not self._sentinel.is_set():
            try:
                ...
                if not reconnect:
                    if keyboard.is_pressed('alt'):
                        print("Connecting...")
                        with BleDataConnection(self.collector, self.collector.myAddr, min_conn_interval_ms=1_000, max_conn_interval_ms=1_000, conn_sup_timeout_ms=4_000, discover_handles=True) as client:
                            print("Connected!")
                            self.logger.info(f"Connected to SMP290")
                            connected = True
                            #time.sleep(3)
                            
                            # Enter state machine
                            state_machine = ConnectedStateMachine(self.collector, client, self.logger)
                            state_machine.run()

                        #time.sleep(3)
                        print("Disconnected!")
                        self.logger.info(f"Disconnected from SMP290")
                        connected = False
                        #time.sleep(3)
            ...
            #finally:
                #if connected:
                    #self.collector.disconnect(myConn)
                    #connected = False
            time.sleep(1)
        self._ClearUp()
